...
var create_client = function (token, config) {
     ...
     * warmup_feature
     * ---
     * Warmup data for future use of get_feature
     *
     * featureName:string                  feature id / key / name
     * distinct_id:string                  unit identified, i.e. user id
     */
    metrics.warmup_feature = function (featureName, distinct_id) {
        metrics.send_request({
                method: "POST", endpoint: '/warmup_feature', data: {
            ...
            function (err) {
                if (err) {
                    console.error('Kibotu error warmup: ', err);
                }
            });
    }
     ...
     *                                     finished or an error occurs
     */
    metrics.get_feature = function (featureName, distinct_id, propOverrides, callback) {
        metrics.send_request({
