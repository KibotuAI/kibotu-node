...
var create_client = function (token, config) {
    ...
    metrics.send_request = function (options, callback) {
        ...
        if (options.requestTimeout) {
            request.setTimeout(options.requestTimeout, () => {
                if (metrics.config.verbose) {
    ...
    metrics.get_personalized_banner = function (userId,
        ...
        metrics.send_request({
                ...
                requestTimeout: allowedTimeout || 5000,
                data: {
                    userId: userId,
                    activeChamp: params?.activeChamp,
                    offeredChamp: params?.offeredChamp,
                    offeredSkin: params?.offeredSkin,
                    offerId: params?.offerId,
                    epicness: params?.epicness,
                    prizeTypes: params?.prizeTypes,
