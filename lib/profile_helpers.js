...
exports.ProfileHelpers = (Base = Object) => class extends Base {
    get token() {
        return this.kibotu.token;
    }

    get config() {
        return this.kibotu.config;
    }
    ...
    _set(prop, to, modifiers, callback, {identifiers, set_once = false}) {
        ...
        if (this.config.debug) {
            console.log(`Sending the following data to Kibotu (${this.endpoint}):`);
            console.log(data);
        }

        this.kibotu.send_request({ endpoint: this.endpoint, data }, callback);
    }
